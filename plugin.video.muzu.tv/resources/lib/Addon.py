...
def add_video_item(_url, infolabels, img='', fanart='', total_items=0, playlist=None):
    ...
    infolabels = decode_dict(infolabels)
    isPlayable = 'false' if (url == '') else 'true'
    if url.find('://') == -1:
    ...
    if playlist != None:
        ...
        #title = urllib.quote(infolabels['title'].encode('ascii','ignore'))        
        title = infolabels['title']
        
        contextMenuItems.append(('Download',       'XBMC.RunPlugin(%s?mode=download&play=%s)'     % (plugin_url, title.replace('&', 'and') +'@'+_url)))
        contextMenuItems.append(('Add to Library', 'XBMC.RunPlugin(%s?mode=add_library&play=%s)'  % (plugin_url, title.replace('&', 'and') +'@'+_url)))
